...
Since we're discussing software development, let me comment on what to do when things go wrong. At some point you will find that a script has not shut down properly, and you will need to manually purge running or wedged processes. I have found the following utilities useful (put in your .bashrc) useful:
...
If your model is resident on a GPU (courtesy a call to `model = model:cuda()`), then the two tensors `params` and `gradParams` are CudaTensors and resident on the same GPU, and can now use `nccl` to transfer data directly between instances of them using functions such as `Bcast, `Reduce`, `AllReduce`, etc. 
